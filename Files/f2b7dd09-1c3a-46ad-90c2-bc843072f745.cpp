...
void Display3() {
	...
	for (double x = 0; x <= xmax; x += ratia) {
		double x1, y1, dist;
		int closest_int;
		closest_int = (x - int(x) >= 0.5) ? int(x) + 1 : int(x);
		dist = fabs(x - closest_int);
		x1 = x / xmax;
		y1 = (x > 0) ? dist / x : 1;
...
//Melcul lui Pascal
//y = (-1, 1) nu e nevoie sa calculam maximul functiei
void Display4() {
	double x, y, t;
	double a = 0.3;
	double b = 0.2;
